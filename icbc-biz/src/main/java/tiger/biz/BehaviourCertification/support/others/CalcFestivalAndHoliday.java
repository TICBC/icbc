 ...
 * @author liu 2015-1-5 
 */  
public abstract class CalcFestivalAndHoliday {  
    ...
    public static String solarToLunar(String solarDate) throws Exception{  
        ...
        if(lunarMonth<10)
        {
        	result =String.valueOf(lunarYear)+"0"+String.valueOf(lunarMonth);
        }
        else
        {
        	result =String.valueOf(lunarYear)+String.valueOf(lunarMonth);
        }
        if(lunarDay<10)
        {
        	result=result+"0"+String.valueOf(lunarDay);
        }
        else
        {
        	result=result+String.valueOf(lunarDay);
        }
        return result;
    }  
     ...
     * @throws Exception
     */
    public static boolean checkFestival(String date)
    {
    	boolean isfes =false;
		try {
			date=date.replace("-","");
			String month_and_day=date.substring(4);
			String lunardate= CalcFestivalAndHoliday.solarToLunar(date);
			String lunar_month_and_day=lunardate.substring(4);
			isfes = false;
			String lunarfestival[]={"1230","0101","0102","0103","0104","0105","0106","0115","0505","0815"};//阴历节日：春节，元宵节，端午节，中秋节
			String solarfestival[]={"0101","0404","0501","1001","1002","1003","1111","1225"};//元旦，清明节，劳动节，国庆节，双十一，圣诞节
			for(int i=0;i<solarfestival.length;i++)
			{
				if(month_and_day.equals(solarfestival[i]))
					isfes=true;
			}
			for(int i=0;i<lunarfestival.length;i++)
			{
				if(lunar_month_and_day.equals(lunarfestival[i]))
					isfes=true;
			}
		} catch (Exception e) {
			e.printStackTrace();
     ...
     * @return 是否属于节假日
     */
    public static boolean checkFestival(Date date){
    	SimpleDateFormat sdf=new SimpleDateFormat("yyyy-MM-dd");
    	String dateStr = sdf.format(date);
    	return checkFestival(dateStr);
    }
